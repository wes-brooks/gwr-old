...
eta = X1*B1 + X2*B2 + X5*B5 + rnorm(N**2, 0, 0.1)
Z = rnorm(N**2, 0, 1)
#p = exp(eta) / (1+exp(eta))
Y = rnorm(N**2, eta, 1)
...
bw = gwlars.sel(Y~X1+X2+X3+X4+X5+Z, data=sim, coords=sim[,c('loc.x','loc.y')], mode.select="AIC", weights=rep(1, N**2), gweight=bisquare, tol=0.01, s=NULL, mode='step', method='knn', parallel=TRUE, longlat=FALSE, adapt=TRUE, precondition=FALSE)
...
model = gwlars(Y~X1+X2+X3+X4+X5+Z-1, data=sim, coords=sim[,c('loc.x','loc.y')], mode.select='CV', bw=bw, weights=rep(1, N**2), gweight=bisquare, tol=0.01, s=NULL, mode='step', method='knn', parallel=FALSE, longlat=FALSE, adapt=TRUE, precondition=FALSE)
