...
xtable.printbold(xtable(msex.table, digits=3, align=rep('c', length(sim.modes)+3), caption="Mean squared error of $\\hat{\\beta_1}$ (\\textbf{minimum}, \\emph{next best}).\\label{MSEX}"), which.bold=msexbold, which.ital=msexital, include.rownames=FALSE, hline.after=c(0))
