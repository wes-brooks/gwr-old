gwglmnet.nen.adaptive.fit = function(x, y, coords, D, s, verbose, family, prior.weights, gweight, target, beta1, beta2, type='pearson', tol=1e-25, longlat=FALSE) {
    ...
    for(i in 1:n) {
        ...
        result[[i]] = list(model=model, cv.error=cv.error, s=s.optimal, index=i)
    }

    print("returning from gwglmnet.nen.adaptive.fit.parallel")

    class(result) = 'gwglmnet.object'
    return(result)
}
