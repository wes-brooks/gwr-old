...
xtable.printbold(xtable(msex.table, digits=3, align=rep('c', length(sim.modes)+3), caption"Mean squared error of $\hat{\beta_1}$ (\textbf{minimum}, \emph{next best}).\label{table:X1-mse}"), which.bold=msexbold, which.ital=msexital, include.rownames=FALSE, hline.after=c(0))
...
xtable.printbold(xtable(bx.table, digits=3, align=rep('c', length(sim.modes)+3), caption="Bias of $\hat{\beta_1}$ (\textbf{minimum}, \emph{next best}).\label{table:X1-bias}"), which.bold=bxbold, which.ital=bxital, include.rownames=FALSE, hline.after=c(0))
...
selection.table = matrix(NA, nrow=2*length(locs), ncol=0)
for (j in 1:3) {    
    for (m in selection.modes) {
        selection.block = matrix(NA, nrow=0, ncol=2)
        for (l in 1:length(locs)) {
            for (i in 1:2) {
                selection.block = rbind(selection.block, c(selected[[l]][[m]][['X1']][(j-1)*2 + i], mean(sapply(vv[-1], function(x) {selected[[l]][[m]][[x]][(j-1)*2 + i]}))))
            }
        }
