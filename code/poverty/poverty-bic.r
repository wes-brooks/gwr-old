...
for (year in c(1960, 1970, 1980, 1990, 2000, 2006)) {
    ...
    bw[[as.character(year)]] = gwglmnet.sel(formula=f, data=df, family='gaussian', alpha=1, coords=df[,c('x','y')], longlat=TRUE, mode.select="BIC", gweight=bisquare, tol=0.01, s=NULL, method='dist', adapt=TRUE, precondition=FALSE, parallel=TRUE, interact=TRUE, verbose=TRUE, shrunk.fit=FALSE, AICc=TRUE)
    model[[as.character(year)]] = gwglmnet(formula=f, data=df, family='gaussian', alpha=1, coords=df[,c('x','y')], longlat=TRUE, N=1, mode.select='BIC', bw=bw[[as.character(year)]], gweight=bisquare, tol=0.01, s=NULL, method='dist', simulation=TRUE, adapt=TRUE, precondition=FALSE, parallel=TRUE, interact=TRUE, verbose=FALSE, shrunk.fit=FALSE, AICc=TRUE)
    ...
    #dev.new()
    pdf(paste('figures/poverty/', year, '.linear.coefficients-bic.pdf', sep=''), width=8, height=16)
    ...
    #dev.new()
    pdf(paste('figures/poverty/', year, '.linear.coefficients.unshrunk-bic.pdf', sep=''), width=8, height=16)
    grid.arrange(pp[['pag']], pp[['pex']], pp[['pman']], pp[['potprof']], pp[['pfire']], pp[['pserve']], pp[['pwh']], pp[['pblk']], pp[['phisp']], pp[['metro']], ncol=2)
    dev.off()
