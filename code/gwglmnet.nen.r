gwglmnet.nen <- function(formula, data, coords, gweight, bw, D=NULL, verbose=FALSE, longlat=FALSE, adapt=FALSE, s=NULL, family, weights=NULL, tolerance=.Machine$double.eps^0.25, beta1=NULL, beta2=NULL, type, parallel=FALSE) {
    ...
    if (adapt) {
        if (parallel) {
            res[['model']] = gwglmnet.nen.adaptive.fit.parallel(x, y, coords, D, s, verbose, family, weights, gweight, bw, beta1, beta2, type, tol=toelrance, longlat=longlat)
        } else {
            res[['model']] = gwglmnet.nen.adaptive.fit(x, y, coords, D, s, verbose, family, weights, gweight, bw, beta1, beta2, type, tol=toelrance, longlat=longlat)
        }
    }
    if (!adapt) {        
        if (!parallel) {
