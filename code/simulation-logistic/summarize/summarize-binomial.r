...
for (setting in settings) {
    ...
    for (k in sims[[setting]]) {
        sim = (setting-1)*100 + as.numeric(k)
        cat(paste("Begin simulation ", sim, ".\n", sep=""))

        #Get the raw data
        filename = paste(root, sim, "/Data.", cluster, ".", sim-1, ".csv", sep="")
        if (file.exists(filename)) {
            raw = read.csv(filename, header=TRUE)

            for (m in sim.modes) {
                filename = paste(root, sim, "/CoefEstimates.", cluster, ".", sim-1, file.endings[[m]], sep="")
                ...
                if (!is.na(pmatch("unshrunk", m))) {
                    ...
                    filename = paste(root, sim, "/MiscParams.", cluster, ".", sim-1, file.endings[[m]], sep="")
