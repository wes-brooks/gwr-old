...
Estimating coefficients for a spline-based VCR model is similar to fitting a generalized additive model (GAM), except that the design matrix, which for a GAM is calculated by decomposing the covariates via a spline basis, is further multiplied by a diagonal matrix of the effect-modifying variable to produce a VCR \citep{Wood:2006}. A local likelihood can also be used to fit generalized linear models with varying coefficients using kernel smoothing \citep{Loader:1999}. \cite{Fan:1999} demonstrated that the optimal kernel bandwidth estimate for a VCR model can be found via a two-step technique.
...
\section{Geographically weighted regression \label{section:GWR}}
	...
	\end{align}
	From (\ref{eq:local-likelihood}) and (\ref{eq:local-log-likelihood}), the GWR coefficient estimates $\hat{\bm{\beta}}_{i,\text{GWR}}$ are obtained by maximizing the local likelihood at location $\bm{s}_i$. It is apparent that can this be accomplished using weighted least squares. 
...
The coefficient estimates are plotted on maps of the upper midwest in Figure \ref{fig:unshrunk-enet-coefs-1970} (based on the GWEN-LLE) and Figure \ref{fig:gwr-coefs-1970} (based on traditional GWR).

	\begin{figure}
		\begin{center}
			\includegraphics[height=8in]{../../figures/practice-talk/poverty-response}
