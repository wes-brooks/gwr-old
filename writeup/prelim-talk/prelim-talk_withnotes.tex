...
We want to understand how the poverty rate is related to these variables that describe the economic structure.}
\end{frame}
...
This model relies on a priori global variable selection.}
\end{frame}
...
\begin{itemize}
    ...
    \begin{align*}
        E\{Y(\bm{s})\} = \beta_1(\bm{s}) X_1(\bm{s}) + \cdots + \beta_p(\bm{s}) X_p(\bm{s})
    \end{align*}
\end{itemize}

\note{Splines are a method of parameterizing smooth functions and it is possible to use splines to represent the coefficients in a varying coefficient regression model. There is a good overview of this topic in in Simon Wood's 2006 book.
...
\note{There are at least two references that describe how to select the covariates for a spline-based VCR model. Both use a similar regularization technique.
