...
The GWEN uses the adaptive elastic net for model selection, which has an oracle property under suitable conditions.
...
The geographically weighted adaptive lasso (GWAL) is a particular case of the GWEN, with no $\ell_2$ penalty it uses the adaptive lasso for model selection, which also has an oracle property under suitable conditions.
...
\note{Model selection via the adaptive elastic net requires selecting the tuning parameter $\lambda$. For the GWEN, $\lambda$ is selected by a BIC.
...
To reduce the boundary effect, the GWEN and GWAL can be fit with coefficient estimates that are locally linear, rather than locally constant. This is done after model selection by augmenting the selected covariates with covariate-by-coordinate interactions.
...
\note{In order to assess the utility of the GWEN for model selection and coefficient estimation in a varying coefficients model, I performed a simulation study.
...
The covariates were simulated at two levels of collinearity: none, and moderate, for which the Pearson correlation was set to 0.5}
\end{frame}
...
First we will consider the model selection results of the simulation.}
\end{frame}
