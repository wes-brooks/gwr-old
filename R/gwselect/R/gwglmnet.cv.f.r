gwglmnet.cv.f = function(formula, data, weights, indx, family, bw, coords, gweight, mode, mode.select, verbose, adapt, longlat, s, tol, method, N, parallel, precondition, shrink) {    
    ...
    gwglmnet.model = gwglmnet(formula=formula, data=data, family=family, weights=weights, tuning=TRUE, indx=indx, coords=coords, gweight=gweight, bw=bw, mode=mode, N=N, mode.select=mode.select, verbose=verbose, longlat=longlat, adapt=adapt, s=s, method=method, parallel=parallel, precondition=precondition, shrink=shrink)
