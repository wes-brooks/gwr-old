gwlars.fit.fixedbwparallel = function(x, y, coords, indx, fit.loc, bw, D=NULL, N, s=NULL, mode.select, tuning, predict, simulation, oracle, verbose=FALSE, gwr.weights=NULL, prior.weights=NULL, gweight=NULL, longlat=FALSE, adapt=FALSE, precondition=FALSE, interact, shrunk.fit, AICc) {
    ...
    models = foreach(i=1:n, .packages=c('lars'), .errorhandling='remove') %dopar% {
        ...
        if (is.null(oracle)) {
            m = gwlars.fit.inner(x=x, y=y, bw=bw, coords=coords, loc=loc, indx=indx, N=N, s=s, mode.select=mode.select, tuning=tuning, predict=predict, simulation=simulation, verbose=verbose, gwr.weights=gw, prior.weights=prior.weights, gweight=gweight, precondition=precondition, interact=interact, shrunk.fit=shrunk.fit, AICc=AICc)
        } else {
        ...
        if (verbose) {
        	cat(paste("For i=", i, "; location=(", paste(round(loc,3), collapse=","), "); bw=", round(bw,3), "; loss=", round(m[['loss.local']],3), "; s=", m[['s']], "; sigma2=", round(tail(m[['sigma2']],1),3), "; nonzero=", paste(m[['nonzero']], collapse=", "), ".\n", sep=''))
        }
        return(m)
    }
