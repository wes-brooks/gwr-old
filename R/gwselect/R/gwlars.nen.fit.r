gwlars.nen.fit = function(x, y, coords, D, s, verbose, prior.weights, gweight, target, beta1, beta2, tol=1e-25, longlat=FALSE, adapt) {
    ...
    for (i in 1:n) {
        ...
        gwlars.object[[i]] = list(model=model, cv.error=cv.error, s=s.optimal, index=i)
    }
