gwlars <- function(formula, data, weights=NULL, coords, indx=NULL, fit.loc=NULL, gweight, D=NULL, bw=NULL, N=1, verbose=FALSE, longlat, tol, method, tuning=FALSE, predict=FALSE, simulation=FALSE, adapt=FALSE, s=NULL, mode.select="CV", shrink=TRUE, mode='step', parallel=FALSE, precondition=FALSE) {
    if (!is.logical(adapt)) 
    ...
    if (method=='dist') {
        ...
        if (parallel) {
            res[['model']] = gwlars.fit.fixedbwparallel(x=x, y=y, prior.weights=weights, coords=coords, indx=indx, fit.loc=fit.loc, D=D, bw=bw, N=N, gwr.weights=weight.matrix, s=s, mode.select=mode.select, tuning=tuning, predict=predict, simulation=simulation, shrink=shrink, mode=mode, verbose=verbose, adapt=adapt, precondition=precondition)
        } else {
            res[['model']] = gwlars.fit.fixedbw(x=x, y=y, prior.weights=weights, coords=coords, indx=indx, fit.loc=fit.loc, D=D, bw=bw, N=N, gwr.weights=weight.matrix, s=s, mode.select=mode.select, tuning=tuning, predict=predict, simulation=simulation, shrink=shrink, mode=mode, verbose=verbose, adapt=adapt, precondition=precondition)
        }
    } else {        
